...
<template>
  <div class="flex h-full flex-col items-center justify-center px-12 py-8">
    <div class="mb-10 flex flex-col items-center text-center">
      <NuxtLink to="/">
        <NuxtImg src="/DA_Logo.png" class="h-25" />
      </NuxtLink>
    ...
    <form @submit.prevent="submitForm" class="flex w-full max-w-sm flex-col gap-6">
      <div v-if="errors.general"
        class="bg-destructive/10 text-destructive rounded-xl p-4 text-center text-sm font-medium">
        {{ errors.general }}
      </div>

      <AuthInput id="email" v-model="form.email" label="Email Address" type="email" :error="errors.email" />
      <AuthInput id="password" v-model="form.password" label="Password" type="password" :error="errors.password" />

      <div class="flex items-center justify-between px-1">
        <label class="text-foreground/60 flex items-center gap-2 text-sm">
          <input type="checkbox" class="accent-primary border-input h-4 w-4 rounded" />
          Remember me
        </label>
    ...
    <div class="mt-10 text-center text-sm">
      ...
      <NuxtLink to="/auth/register" class="text-primary ml-1 font-semibold hover:underline">
        Create one
      </NuxtLink>
    </div>
  </div>
