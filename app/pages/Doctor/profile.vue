...
watch(user, (newVal) => {
  if (newVal?.data && !loaded.value) {
    ...
    if (userData.doctor_verification) {
      form.prc_number = userData.doctor_verification.prc_number || ''
    }
...
<template>
  <div class="max-w-4xl">
    <div class="mb-8 flex items-center justify-between">
      ...
      <div v-if="user?.doctor_verification?.is_verified"
    ...
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
      ...
      <div class="lg:col-span-1">
        <div class="bg-sidebar/40 border-sidebar-border rounded-3xl border p-6 text-center shadow-sm backdrop-blur-sm">
          <div
            class="relative mx-auto mb-4 h-32 w-32 overflow-hidden rounded-full bg-gradient-to-br from-primary/20 to-primary/5 p-1 border-2 border-primary/20 text-sidebar">
            <div
            ...
            <button
              class="absolute bottom-0 right-0 bg-primary p-2 rounded-full text-white shadow-lg hover:bg-primary-hover transition-colors">
              <Icon name="heroicons:camera-20-solid" size="16" />
            </button>
          </div>
          <h2 class="text-xl font-bold italic">Dr. {{ form.first_name }} {{ form.last_name }}</h2>
          <p class="text-foreground/60 text-sm">{{ form.email }}</p>

          <div class="mt-6 space-y-3">
            <div class="flex items-center justify-between text-sm">
              <span class="text-foreground/50">Profile Setup</span>
              <AppProfileStatusBadge
                :is-complete="!!(user?.data?.city && user?.data?.province && user?.data?.age && user?.data?.gender)" />
            </div>
            <div v-if="form.prc_number" class="bg-foreground/5 rounded-xl p-3 text-left">
              <span class="text-foreground/40 text-[10px] font-bold uppercase block mb-1">PRC License No.</span>
              <span class="text-sm font-mono">{{ form.prc_number }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2">
        <div class="bg-sidebar border-sidebar-border rounded-3xl border p-8 shadow-sm text-sidebar-foreground">
          <form @submit.prevent="submitProfile" class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <input v-model="form.prc_number" type="text" disabled
                  class="bg-foreground/5 border-sidebar-border w-full rounded-2xl border px-4 py-3 outline-none transition-all opacity-60 cursor-not-allowed"
                  placeholder="Verified license" />
              </div>
            </div>
