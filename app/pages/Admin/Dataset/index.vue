...
<template>
  <div class="">
    ...
    <div v-else class="space-y-12">
      <div v-for="dataset in datasets" :key="dataset.category"
        ...
        <div class="space-y-4">
          <div>
            ...
            <select v-model="uploadCategory"
              class="w-full rounded-xl border-gray-300 shadow-sm focus:border-primary focus:ring-primary p-3 border">
    ...
    <AppModal v-model="modalState.isOpen" :title="modalState.title" :description="modalState.description" size="md">
      <template #footer>
        <AppButton v-if="modalState.actionVariant === 'destructive'" variant="ghost" @click="modalState.isOpen = false">
          Cancel</AppButton>
        <AppButton :variant="modalState.actionVariant" @click="modalState.onConfirm">{{ modalState.actionText }}
        </AppButton>
      </template>
    </AppModal>
  </div>
