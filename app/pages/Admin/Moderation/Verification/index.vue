...
<template>
  <div class="px-6 py-8">
    ...
    <div v-if="verifications?.data?.length" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <TransitionGroup name="grid">
        <NuxtLink v-for="verification in verifications.data" :key="verification.id"
          ...
          class="group relative block overflow-hidden rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
          <!-- Status Badge -->
          ...
          <div class="flex flex-col items-center text-center">
            <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600">
    ...
    <div v-else-if="pending" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <div v-for="i in 4" :key="i" class="h-[480px] animate-pulse rounded-xl bg-gray-100" />
    </div>
  </div>
</template>
