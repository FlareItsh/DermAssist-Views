...
<template>
  <div class="bg-card w-[400px] rounded-xl border border-gray-100 p-6">
    <div class="mb-4 flex items-center justify-between">
      ...
      <div class="flex gap-2">
        <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="prevMonth"
        ...
        <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="nextMonth"
          class="hover:bg-primary/10 flex h-8 w-8 items-center justify-center rounded-full transition-colors active:scale-95">
    ...
    <div class="grid grid-cols-7 gap-y-4 text-center">
      <div v-for="day in daysOfWeek" :key="day" class="text-[12px] font-bold tracking-wider text-gray-400 uppercase">
        {{ day }}
      </div>
      ...
      <div v-for="date in daysInMonth" :key="date" @click="selectDate(date)"
        ...
        <div v-if="hasAppointment(date)" class="bg-secondary absolute -bottom-1 h-1 w-1 rounded-full"></div>
      </div>
    </div>
  </div>
</template>
