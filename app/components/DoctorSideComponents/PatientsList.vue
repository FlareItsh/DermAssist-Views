...
<template>
  <div class="bg-card rounded-xl border border-gray-100 p-5">
    ...
    <div class="flex gap-4 overflow-x-auto pb-2 custom-scrollbar snap-x snap-mandatory">
      <div v-if="filteredPatients.length === 0" class="w-full py-10 text-center text-muted-foreground italic text-sm">
      ...
      <div v-for="patient in filteredPatients" :key="patient.id"
        ...
        <div class="mt-0.5 flex items-center justify-between gap-1">
          ...
          <div class="flex gap-1 shrink-0">
            ...
            <AppButton variant="unstyled" size="unstyled" rounded="unstyled"
              @click.stop="handleAddToPriority(patient.id)" :disabled="isInPriority(patient.id)"
              ...
              <Icon :name="isInPriority(patient.id) ? 'material-symbols:check-rounded' : 'fluent:add-12-filled'"
