  ...
  const parseMessageParts = (text: string) => {
    ...
    while ((match = urlRegex.exec(text)) !== null) {
      ...
      if (url.includes('meet.google.com')) {
        ...
        bgColor = 'bg-white'
        textColor = 'text-gray-800 border border-gray-200'
      ...
      parts.push({
        ...
        url: url,
        icon: icon,
        classes: `inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-sm font-bold shadow-sm transition-all hover:-translate-y-0.5 ${bgColor} ${textColor} no-underline mt-1 mb-1 whitespace-normal break-all`
      })
        ...
        >
          ...
          <div class="flex max-w-[70%] flex-col gap-1">
            ...
            >
              ...
              <p v-else-if="msg.message" class="text-base leading-relaxed whitespace-pre-wrap">
                <template v-for="(token, index) in parseMessageParts(msg.message)" :key="index">
                  ...
                  <a 
                    ...
                    :class="token.classes"
                  >
                    <Icon :name="token.icon || 'lucide:link'" class="text-lg" />
                    {{ token.url }}
                  </a>
                </template>
              </p>
