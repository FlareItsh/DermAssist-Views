  ...
  <div class="bg-card flex h-full flex-col">
    ...
    <div class="border-border flex items-center justify-between border-b px-8 py-6">
      <div class="flex items-center gap-4">
        <NuxtLink :to="$route.path.replace(/\/[^/]+$/, '')"
        ...
        <div class="bg-muted h-12 w-12 overflow-hidden rounded-full">
          <img v-if="otherPersonAvatar" :src="otherPersonAvatar" :alt="otherPersonName"
      ...
      <!-- Header actions menu -->
      <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="showDeleteConversationModal = true"
    ...
    <div v-if="activeAppointment" class="border-b p-4 flex items-center justify-between transition-all" :class="[
      ...
      <div class="flex items-center gap-3">
        <div class="p-2 rounded-full" :class="isToday(activeAppointment.date) ? 'bg-amber-100' : 'bg-indigo-100'">
          <Icon
            ...
            class="text-xl" :class="isToday(activeAppointment.date) ? 'text-amber-600' : 'text-indigo-600'" />
        </div>
        <div>
          ...
          <p class="text-xs font-medium"
            ...
            {{ isToday(activeAppointment.date) ? 'Your appointment is scheduled for today' : activeAppointment.date }}
      ...
      <div v-if="userRole?.toLowerCase() === 'doctor'">
        <AppButton @click="showCompleteConfirm = true"
          class="bg-indigo-600 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-md hover:shadow-lg flex items-center gap-2">
    ...
    <div ref="messagesContainer" @scroll="handleScroll"
      ...
      <!-- Empty state -->
      <div v-else-if="allMessages.length === 0" class="flex flex-1 items-center justify-center">
        <div class="text-foreground/20 text-center">
      ...
      <div class="flex flex-col gap-4">
        <div v-for="msg in allMessages" :key="msg.id" class="group flex items-end gap-3"
          ...
          <div class="border-border/50 h-10 w-10 shrink-0 overflow-hidden rounded-full border">
            <img v-if="msg.sender?.avatar" :src="msg.sender.avatar" :alt="msg.sender?.name"
              class="h-full w-full object-cover" />
            <div v-else
          ...
          <div class="flex max-w-[70%] flex-col gap-1">
            ...
            <div v-if="editingMessageId === msg.id" class="flex flex-col gap-2">
              <textarea v-model="editingMessageText" @keydown="handleEditKeydown"
            ...
            <!-- Normal bubble -->
            <div v-else class="relative px-5 py-3 transition-all" :class="[
              ...
              <div v-if="msg.message.includes('[APPOINTMENT_REQUEST:') || msg.message.includes('[DIAGNOSIS_ONLY:')">
                <div class="mb-2">
                  ...
                  <p class="text-sm opacity-80">{{
                    msg.message.replace(/\[(APPOINTMENT_REQUEST|DIAGNOSIS_ONLY):.*?:.*?\]/g, '').trim() ||
                    (msg.message.includes('[APPOINTMENT_REQUEST:') ? 'A diagnosis was shared.' : 'Additional findings
                    shared.') }}</p>
                </div>

                <!-- Diagnosis Details -->
                <div v-if="msg.appointment_data"
                  class="bg-card/50 mt-3 rounded-2xl border border-border/50 p-4 shadow-sm backdrop-blur-sm">
                  <div class="flex gap-4">
                    <img :src="getStorageUrl(msg.appointment_data.diagnosis.image_path)"
                      class="h-24 w-24 rounded-xl object-cover border border-border shadow-sm" alt="Diagnosis scan" />
                    <div class="flex flex-col justify-center gap-0.5">
                      <p class="text-[10px] font-black uppercase tracking-widest text-indigo-500">Clinical Findings</p>
                      <h4 class="text-lg font-black leading-tight text-foreground">{{
                        msg.appointment_data.diagnosis.label }}</h4>
                ...
                <div
                  v-if="userRole?.toLowerCase() === 'doctor' && msg.message.includes('[APPOINTMENT_REQUEST:') && !isAppointmentHandled(msg.message) && !activeAppointment"
                  class="flex gap-2 mt-4">
                  <button @click.prevent="openScheduleModal(msg.message)"
                    class="bg-indigo-600 text-white px-5 py-2 rounded-xl text-sm font-bold shadow-md hover:bg-indigo-700 transition-all active:scale-95">
                    Accept & Schedule
                  </button>
                  <button @click.prevent="declineAppointment(msg.message)"
                    class="bg-foreground/10 px-5 py-2 rounded-xl text-sm font-bold hover:bg-foreground/20 transition-all active:scale-95">
                    Decline
                  </button>
                </div>
              </div>
              <div v-else-if="msg.message.includes('[APPOINTMENT_SCHEDULED:')">
                <div class="flex flex-col">
                  ...
                  <p class="text-sm opacity-90"
                    v-html="msg.message.replace(/\[APPOINTMENT_SCHEDULED:.*?\]/g, '').trim()"></p>
                </div>
              </div>
              <div v-else-if="msg.message.includes('[APPOINTMENT_DECLINED:')">
                <div class="flex flex-col">
                  ...
                  <p class="text-sm opacity-90">{{ msg.message.replace(/\[APPOINTMENT_DECLINED:.*?\]/g, '').trim() }}
                  </p>
                </div>
              </div>
              ...
              <button v-if="msg.sender?.id === userUuid" @click.stop="openContextMenu($event, msg)"
                ...
                <Icon name="solar:menu-dots-vertical-bold"
                  class="text-foreground/30 hover:text-foreground/60 text-lg" />
              </button>
            </div>

            <span class="text-foreground/30 text-[11px]"
    ...
    <div class="p-8 pt-0">
      <div class="group relative flex items-end">
        <textarea v-model="messageTerm" placeholder="Type a message..." @keydown="handleKeydown"
          class="bg-foreground/5 focus:border-primary/20 focus:ring-primary/5 custom-scrollbar h-14 w-full resize-none rounded-2xl border border-transparent px-6 pr-16 py-4 text-base transition-all outline-none focus:ring-4"></textarea>

        <div class="absolute top-1/2 right-4 -translate-y-1/2">
          <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="sendMessage"
    ...
    <Teleport to="body">
      <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
        ...
        <div v-if="contextMenu.visible"
          ...
          <button @click.stop="startEditing(contextMenu.message!)"
          ...
          <button @click.stop="confirmDeleteMessage(contextMenu.message!)"
      ...
      <Transition name="modal">
        <div v-if="showDeleteMessageModal"
          ...
          <div
            ...
            <div class="flex flex-col gap-3">
              <AppButton variant="solid" class="border-none bg-red-500 text-white hover:bg-red-600"
                @click="deleteMessage">
                Yes, Delete
              </AppButton>
      ...
      <Transition name="modal">
        <div v-if="showDeleteConversationModal"
          class="bg-foreground/40 fixed inset-0 z-999 flex items-center justify-center p-4"
          ...
          <div
            ...
            <p class="text-foreground/60 mb-8 text-sm">
              Are you sure you want to delete this entire conversation with <strong>{{ otherPersonName
                }}</strong>?<br />
      ...
      <AppModalAppointmentSchedule v-if="showScheduleModal" :appointment-uuid="schedulingAppointmentUuid"
        @close="showScheduleModal = false" @scheduled="() => { fetchMessages(1); fetchAppointments(); }" />
      ...
      <Transition name="modal">
        <div v-if="showCompleteConfirm"
          ...
          <div
            class="modal-container bg-card border-border w-full max-w-md overflow-hidden rounded-xl border p-8 shadow-2xl">
            ...
            <div class="flex flex-col gap-3">
              <AppButton variant="solid" class="bg-indigo-600 text-white hover:bg-indigo-700" :disabled="isCompleting"
