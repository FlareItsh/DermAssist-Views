...
<template>
  <div class="bg-card flex h-full flex-col" v-if="conversation">
    <div class="border-border flex items-center justify-between border-b px-8 py-6">
      ...
      <AppButton variant="unstyled" size="unstyled" rounded="unstyled"
        ...
        <Icon name="solar:menu-dots-vertical-bold" class="text-3xl" />
      </AppButton>
    </div>

    <!-- Active Appointment Bar -->
    <div v-if="activeAppointment && activeAppointment.status === 'scheduled'"
      class="border-b p-4 flex items-center justify-between transition-all" :class="[
    ...
    <div class="custom-scrollbar flex flex-1 flex-col gap-6 overflow-y-auto p-8">
      <div v-for="msg in mockMessages" :key="msg.id" class="flex items-end gap-4"
        ...
        <div class="border-border/50 h-12 w-12 shrink-0 overflow-hidden rounded-full border">
          <img :src="msg.avatar" class="h-full w-full object-cover" />
        </div>

        <div class="max-w-[70%] px-6 py-3 transition-all" :class="[
          msg.sentByMe
    ...
    <div class="p-8 pt-0">
      <div class="group relative flex items-end">
        <textarea v-model="messageTerm" placeholder="Your message" @keydown="handleKeydown"
          class="bg-foreground/5 focus:border-primary/20 focus:ring-primary/5 custom-scrollbar h-17 w-full resize-none rounded-2xl border border-transparent px-15 py-5 text-lg transition-all outline-none focus:ring-4"></textarea>
        ...
        <div class="absolute top-1/2 left-6 -translate-y-1/2">
          <AppButton variant="unstyled" size="unstyled" rounded="unstyled"
            ...
            <Icon name="material-symbols:link-rounded" class="text-foreground/60 text-3xl" />
          </AppButton>
        </div>
        ...
        <div class="absolute top-1/2 right-6 -translate-y-1/2">
          <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="sendMessage"
            ...
            <Icon name="tabler:send" class="text-secondary text-3xl" />
          </AppButton>
        </div>
      </div>
    </div>
  </div>
  <div v-else class="bg-muted/5 flex flex-1 items-center justify-center">
