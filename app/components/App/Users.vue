...
<template>
  <div class="bg-card overflow-hidden rounded-xl border border-gray-100 p-4">
    <div class="mb-4 flex items-center justify-between gap-4">
      ...
      <span v-if="searchQuery" class="text-primary bg-primary/10 rounded-full px-3 py-1 text-xs font-bold italic">
        Filtering by: "{{ searchQuery }}"
      </span>
    </div>

    <div v-if="pending" class="flex min-h-[220px] items-center justify-center p-10">
    ...
    <div v-else-if="filteredUsers.length > 0"
      ...
      <PatientSideComponentsDoctorsCard v-for="userItem in filteredUsers" :key="userItem.id"
        class="shrink-0 cursor-pointer transition-all hover:scale-105 active:scale-95"
        :doctorName="getDisplayName(userItem)" :doctorWorkplace="getWorkplaceOrLocation(userItem)"
    ...
    <div v-else
      ...
      <Icon name="solar:users-group-two-rounded-outline" class="text-foreground/20 mb-3 text-4xl" />
      <p class="text-foreground/60 font-medium">No results match your search</p>
      <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="searchQuery = ''"
