  ...
  <nav aria-label="Quick Actions">
    <ul class="flex items-center gap-5 list-none m-0 p-0">
      <li v-if="isSearchVisible">
        <AppSearch v-model="searchQuery" class="w-70" size="text-3xl" text="text-foreground/50" width="w-9" />
      </li>

      <li class="relative" ref="messageRef">
        <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="toggleMessages"
          ...
          <span v-if="messages.length > 0" class="absolute top-2.5 right-2.5 flex h-3 w-3">
            <span
              class="bg-primary-light absolute inline-flex h-full w-full animate-ping rounded-full opacity-75"></span>
            <span class="bg-primary relative inline-flex h-3 w-3 rounded-full border-2 border-white"></span>
          </span>
        </AppButton>
        ...
        <Transition enter-active-class="transition duration-200 ease-out"
          ...
          <div v-if="isMessagesOpen"
            class="bg-card/90 absolute right-0 z-50 mt-4 max-h-[500px] w-96 overflow-hidden rounded-xl border border-white/20 shadow-2xl backdrop-blur-xl">
            <div class="border-border/50 flex items-center justify-between border-b p-5">
              <h3 class="text-xl font-bold">Messages</h3>
              <div class="flex items-center gap-2">
                <span class="bg-primary/10 text-primary rounded-full px-2 py-1 text-xs font-bold">4 New</span>
            ...
            <div class="custom-scrollbar max-h-[400px] overflow-y-auto">
              <div v-if="messages.length === 0" class="text-muted-foreground p-10 text-center">
                <Icon name="solar:chat-line-linear" class="mx-auto mb-3 text-5xl opacity-20" />
                <p>No messages yet</p>
              </div>
      ...
      <li class="relative" ref="notificationRef">
        <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="toggleNotifications"
          class="flex h-13 w-13 cursor-pointer items-center justify-center rounded-full p-1 transition-all active:scale-95"
          ...
          <span v-if="notifications.length > 0" class="absolute top-3 right-3 flex h-3 w-3">
            ...
            <span class="relative inline-flex h-3 w-3 rounded-full border-2 border-white bg-red-500"></span>
          </span>
        </AppButton>
        ...
        <Transition enter-active-class="transition duration-200 ease-out"
          ...
          <div v-if="isNotificationsOpen"
            ...
            <div class="border-border/50 flex items-center justify-between border-b p-5">
              ...
              <span class="text-primary cursor-pointer text-sm font-medium hover:underline">Mark all as read</span>
            </div>

            <div class="custom-scrollbar max-h-[400px] overflow-y-auto">
              <div v-if="notifications.length === 0" class="text-muted-foreground p-10 text-center">
                <Icon name="solar:bell-bing-linear" class="mx-auto mb-3 text-5xl opacity-20" />
                <p>No new notifications</p>
              </div>
              <ul v-else class="list-none m-0 p-0">
                <li v-for="notif in notifications" :key="notif.id">
                  <AppNotificationPreview v-bind="notif" @click="handleNotificationClick(notif)"
      ...
      <li v-if="userRole !== 'admin'" class="relative" ref="profileRef">
        <button @click="toggleProfile"
          ...
          <NuxtImg :src="getStorageUrl(userProfile?.data?.avatar_path) || '/images/lp-img.png'"
            class="h-full w-full object-cover" alt="Profile" placeholder />
        </button>
        <!-- Profile incomplete dot -->
        <span v-if="isProfileIncomplete" class="pointer-events-none absolute top-0 right-0 flex h-4 w-4">
          <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-red-400 opacity-75"></span>
        ...
        <Transition enter-active-class="transition duration-200 ease-out"
          ...
          <div v-if="isProfileOpen"
            class="bg-card/95 absolute right-0 z-50 mt-4 w-64 overflow-hidden rounded-xl border border-white/20 shadow-2xl backdrop-blur-xl">
            <div class="border-border/10 border-b p-4 pb-3">
              <p class="text-foreground/40 text-[10px] font-bold uppercase">Logged in as</p>
              <p class="text-sm font-bold truncate">{{ userProfile?.data?.first_name }} {{ userProfile?.data?.last_name
                }}</p>
              <p class="text-muted-foreground text-xs truncate">{{ userProfile?.data?.email }}</p>
            </div>

            <div class="p-2">
              <NuxtLink :to="profileRoute" @click="isProfileOpen = false"
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="isLogoutModalOpen"
          ...
          <AppModalLogoutConfirmation @close="isLogoutModalOpen = false" @confirm="logout" />
        </div>
      </Transition>
    </Teleport>
  </nav>
