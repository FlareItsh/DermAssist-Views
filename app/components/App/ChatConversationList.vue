...
<template>
  <div class="bg-card border-border flex h-full w-80 shrink-0 flex-col overflow-hidden rounded-xl border shadow-sm">
    <div class="border-border border-b p-4">
      <div class="relative">
        <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400">
          <Icon name="material-symbols:search-rounded" class="text-xl" />
        </span>
        <input v-model="searchValue" type="text" placeholder="Search conversations..."
    ...
    <div class="custom-scrollbar flex-1 overflow-y-auto pt-2">
      <div v-if="pending && !conversations?.data?.length" class="space-y-3 p-4">
      ...
      <div v-else-if="filteredConversations.length === 0" class="text-muted-foreground p-10 text-center">
        <Icon name="solar:chat-round-line-linear" class="mx-auto mb-2 text-4xl opacity-20" />
        <p v-if="searchValue" class="text-sm">No conversations matching "{{ searchValue }}"</p>
        <p v-else class="text-sm">No conversations yet</p>
      </div>

      <button v-for="conv in filteredConversations" :key="conv.id" @click="handleSelect(conv)"
        ...
        <div class="bg-muted h-14 w-14 shrink-0 overflow-hidden rounded-2xl">
          <img v-if="getOtherPerson(conv)?.avatar" :src="getOtherPerson(conv)!.avatar!"
            :alt="getOtherPerson(conv)?.name || 'User'" class="h-full w-full object-cover" />
          <div v-else
