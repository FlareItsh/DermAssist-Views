...
<template>
  <section class="relative w-full h-full" aria-labelledby="scanner-heading">
    <div class="bg-primary flex h-full flex-col overflow-hidden rounded-xl transition-colors duration-500">
      <div class="sm:mx-10 mb-4 sm:mb-5 flex flex-1 flex-col items-center">
        <h1 id="scanner-heading"
          ...
          <span v-if="isScanning" class="text-destructive uppercase tracking-widest animate-pulse">Analyzing
            Image...</span>
        ...
        <div class="relative flex w-full flex-1 min-h-0 flex-col overflow-hidden rounded-xl rounded-br-none bg-black">
          <img v-if="previewImage" :src="previewImage"
            class="absolute inset-0 h-full w-full rounded-4xl object-contain p-2 sm:p-4 bg-black transition-opacity duration-500"
            :class="{ 'opacity-50': isScanning }" />
          <video v-show="isCameraOn && !previewImage" ref="videoRef" autoplay playsinline
            class="h-full w-full rounded-4xl object-cover p-3 sm:p-5 transition-opacity duration-500 -scale-x-100"
            :class="{ 'opacity-30 pointer-events-none': isScanning }"></video>
          <div v-if="!isCameraOn && !previewImage"
            class="absolute inset-0 flex flex-col items-center justify-center text-gray-500 gap-4 p-4 text-center">
          ...
          <div v-if="previewImage && !isScanning" class="absolute top-8 left-8">
            <AppButton variant="unstyled" size="unstyled" rounded="unstyled" @click="resetToCamera"
              class="bg-white/90 backdrop-blur px-5 py-2 rounded-full font-bold shadow-xl flex items-center gap-2 hover:bg-white active:scale-95 transition-all text-primary">
              <Icon name="material-symbols:arrow-back-rounded" class="text-xl" />
              Retake Scan
            </AppButton>
          </div>
          <div v-if="isCameraOn && !isScanning && !qualityError && !previewImage"
            class="absolute inset-0 border-[30px] sm:border-[60px] border-black/20 pointer-events-none flex items-center justify-center">
            <div class="w-2/3 h-2/3 border-2 border-green-500/50 rounded-xl"></div>
          </div>
          ...
          <AppScannerControls :is-camera-on="isCameraOn || !!previewImage" :is-scanning="isScanning"
            :has-quality-error="!!qualityError" @toggle-camera="toggleCamera" @trigger-file="triggerFileInput"
            @scan="captureAndScan" />
        </div>
      </div>
    </div>
