...
<template>
  ...
  <div class="relative z-50 w-32 shrink-0">
    <aside
      class="border-sidebar-border bg-sidebar absolute top-4 left-4 flex h-[80vh] flex-col justify-between gap-10 overflow-hidden shadow-2xl shadow-black/20 transition-all duration-500 ease-in-out"
      :class="isCollapsed ? 'w-24 rounded-4xl' : 'w-72 rounded-4xl'"
      @mouseenter="isCollapsed = false"
      @mouseleave="isCollapsed = true"
    >
      <div
        class="flex flex-col gap-3 px-4 transition-all duration-500 ease-in-out"
